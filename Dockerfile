...
RUN su - postgres -s /bin/sh -c "/usr/lib/postgresql/15/bin/initdb;/usr/lib/postgresql/15/bin/pg_ctl start"\
...
CMD nginx && su - portgres -s /bin/sh -c "/usr/lib/postgresql/15/bin/pg_ctl start"\

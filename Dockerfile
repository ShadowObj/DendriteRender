...
RUN /usr/lib/postgresql/15/bin/psql;su - postgres -s /bin/sh -c "/usr/lib/postgresql/15/bin/psql"\
...
CMD nginx && su - portgres -s /bin/sh -c "/usr/lib/postgresql/15/bin/psql"\

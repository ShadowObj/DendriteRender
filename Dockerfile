...
RUN su - postgres -c "whoami;whereis psql;which psql"\
#su - postgres -c "whereis psql;/usr/lib/postgresql/15/bin/psql"\
...
CMD nginx && su - portgres -c "/usr/lib/postgresql/15/bin/psql"\

...
# && locale -a\
 ...
 && /usr/lib/postgresql/15/bin/pg_ctl -D /var/lib/postgresql/data -l /dev/null -A 127.0.0.1 start"\
 && su - postgres -s /bin/sh -c "export LANG=en_US.UTF-8;export LC_ALL=en_US.UTF-8;export LC_CTYPE=en_US.UTF-8\
 && createuser dendrite"\
 && su - postgres -s /bin/sh -c "export LANG=en_US.UTF-8;export LC_ALL=en_US.UTF-8;export LC_CTYPE=en_US.UTF-8\
...
CMD nginx && su - portgres -s /bin/sh -c "/usr/lib/postgresql/15/bin/pg_ctl -D /var/lib/postgresql/data -l /dev/null -A 127.0.0.1 start"\
